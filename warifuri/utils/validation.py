...
from ..core.types import Task, TaskInstruction
...
def load_schema(workspace_path: Path) -> Dict[str, Any]:
    ...
    return json.loads(embedded_path.read_text(encoding="utf-8"))
...
def validate_instruction_yaml(
    ...
    try:
        ...
        if e.validator in ("required", "type"):
            raise ValidationError(f"Critical validation error: {e.message}")
