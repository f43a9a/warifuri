...
class AutomationListService:
    ...
    def list_tasks(self, format: str) -> None:
        ...
        ready_tasks = find_ready_tasks(all_tasks)
        ...
        for project in projects:
            for task in project.tasks:
                info = {
                    ...
                    "dependencies": len(task.depends_on) if task.depends_on else 0,
...
class AutomationCheckService:
    ...
    def check_task(self, task_name: str, verbose: bool) -> None:
        ...
        if not task:
            click.echo(f"❌ Task '{task_name_only}' not found in project '{project_name}'", err=True)
            raise click.Abort()
    ...
    def _show_verbose_info(self, task, all_tasks, is_ready: bool) -> None:
        ...
        if task.depends_on:
            ...
            for dep in task.depends_on:
                ...
                if dep_task:
                    dep_ready = dep_task in find_ready_tasks(all_tasks)
        ...
        if not is_ready and task.depends_on:
    ...
    def check_task_automation(self, task_name: str) -> Tuple[bool, List[str], Optional[str]]:
        """Check if a task can be automated. Returns (can_automate, issues, auto_merge_config)."""
        from ...core.types import TaskStatus, TaskType
